...
const image = (
  ...
  // We need to know what the resultant files for each format a size will be called.
  // Let's take a peep.
  for (const [format, options] of Object.entries(metadata)) {
    ...
    metadata[format].forEach(thisSize => {
      ...
      if (fs.existsSync(thisSize.outputPath)) {
        console.log(`Skipping processing for cached image: ${src}`);
      } else {
        console.log(`Processing image: `, thisSize.outputPath);
